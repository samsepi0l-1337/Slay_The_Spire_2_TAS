...
def test_evaluate_model_cli_writes_policy_and_value_metrics(tmp_path: Path, monkeypatch) -> None:
    ...
    write_game_steps(dataset, [anger_step, skip_step, self_labeled])
    ...
    def fake_recommend(_model, step: GameStep) -> Recommendation:
        ...
        return Recommendation(
            ...
            value_score=0.8 if step.chosen_action_id == anger.identity else 0.2,
        )
...
def test_evaluate_model_cli_keeps_deprecated_dataset_alias(tmp_path: Path, monkeypatch) -> None:
    ...
    def fake_recommend(_model, step: GameStep) -> Recommendation:
        ...
        return Recommendation(
            best=CandidateRecommendation(action.identity, action.action_type, action.option_id, 1.0),
            candidates=[CandidateRecommendation(action.identity, action.action_type, action.option_id, 1.0)],
            value_score=0.5,
        )
...
def test_evaluate_play_missing_safety_metrics_fail_closed(tmp_path: Path) -> None:
    episodes = tmp_path / "episodes.jsonl"
    out = tmp_path / "play-eval.json"
    episodes.write_text(json.dumps({"seed": 7, "victory": True, "floor": 20, "hp_remaining": 44, "steps": 12}) + "\n", encoding="utf-8")
    ...
    assert json.loads(out.read_text(encoding="utf-8")) == {
        "average_floor": 20.0,
        "average_hp_remaining": 44.0,
        "average_steps": 12.0,
        "candidate_recall": 0.0,
        "decision_latency_ms": 1000000.0,
        "episodes": 1,
        "illegal_action_rate": 1.0,
        "misclick_rate": 1.0,
        "transition_timeout_rate": 1.0,
        "win_rate": 1.0,
    }


def test_evaluate_play_allow_missing_metrics_reports_missing_count(tmp_path: Path) -> None:
    episodes = tmp_path / "episodes.jsonl"
    out = tmp_path / "play-eval.json"
    episodes.write_text(json.dumps({"seed": 7, "victory": True, "floor": 20, "hp_remaining": 44, "steps": 12}) + "\n", encoding="utf-8")
...
def test_model_metrics_cover_empty_missing_scores_and_value_correlation() -> None:
    ...
    empty = Recommendation(best=CandidateRecommendation("missing", "missing", None, 0.0), candidates=[], value_score=0.0)
    ...
    low_value = Recommendation(
        best=CandidateRecommendation(skip.chosen_action_id, "skip_reward", "skip", 0.8),
        candidates=[CandidateRecommendation(skip.chosen_action_id, "skip_reward", "skip", 0.8)],
        value_score=0.1,
    )

    assert model_evaluation_metrics([])["evaluated_steps"] == 0
    metrics = model_evaluation_metrics([(anger, correct), (skip, low_value)])
    assert metrics["value_correlation"] > 0.0
    assert metrics["legal_action_mask_accuracy"] == 1.0
    assert model_evaluation_metrics([(skip, empty)])["score_margin_incorrect"] == 0.0
