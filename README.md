...
uv run sts2-tas parse-screen --screenshot reward.png --ocr-fixture ocr.json --out parsed.json --ocr-report ocr-report.json
...
`parse-screen --ocr-report ocr-report.json`은 parsed screen JSON과 별도로 unknown token, 낮은 confidence 때문에 버린 catalog 후보, layout region 밖이라 버린 catalog 후보, 간단한 fuzzy 후보를 저장합니다.
