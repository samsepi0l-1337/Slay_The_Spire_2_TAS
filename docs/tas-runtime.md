...
- `sts2-tas memory-snapshot --target-process SlayTheSpire2 --pid PID --memory-registry registry.json --game-version 0.105.1 --branch beta --binary-signature sha256:... --out snapshot.json`: registry offset을 read-only process memory read로 읽어 snapshot JSON을 쓴다.
- `sts2-tas hook-memory-play --hook-jsonl hook.jsonl --hook-session-nonce NONCE --hook-target-pid PID --memory-registry registry.json --memory-snapshot snapshot.json --memory-critical-fields player.hp,player.energy ...`: passive hook IPC event와 read-only memory snapshot을 검증한 뒤 OCR action box와 결합해 한 step을 실행한다.
...
## Hook + Memory Play Path

`hook-memory-play`는 memory 기준 플레이를 v1 live loop와 섞지 않고 별도 진입점으로 제공한다.

구현할 것:

- hook JSONL은 session nonce, target pid, passive-only flag를 검증한다.
- `memory-snapshot`은 Windows `OpenProcess`를 read-only 권한으로 열고 registry offset을 `ReadProcessMemory`로 읽는다.
- memory registry는 game version, branch, binary signature, offset 이름을 검증한다.
- memory snapshot은 `StructuredGameState`로 변환한다.
- OCR-derived state와 memory snapshot을 `--memory-critical-fields`로 cross-check한다.
- critical field mismatch가 있으면 fail-closed report를 출력하고 입력을 보내지 않는다.
- memory가 검증되면 `source_type=memory_cross_checked` state로 action을 계획하고, OCR screen box는 실제 클릭 좌표에만 사용한다.

구현하지 않을 것:

- `WriteProcessMemory`, save memory patching, DLL mutation, RNG/time hook.
- hook DLL에서 입력을 보내는 경로.
- memory cross-check가 실패한 상태에서 gameplay action 실행.

Acceptance 기준:

- `memory_usable=true`, hook nonce/pid/passive 검증 통과, critical mismatch 없음.
- native 실행은 `--target-process`가 지정된 local interactive session에서만 acceptance evidence로 인정한다.
- 현재 macOS/CI 검증은 fake hook JSONL과 snapshot fixture 기반이며, 실제 Windows 게임 5-run evidence를 대체하지 않는다.

테스트 기준:

- `tests/test_hook_memory_play_cli.py`는 success와 mismatch fail-closed를 검증한다.
- `tests/test_tas_hook_ipc.py`는 hook IPC trust boundary를 검증한다.
- `tests/test_memory_play.py`는 read-only registry/snapshot/cross-check 경계를 검증한다.
