...
- `sts2-tas tas-replay --movie run.sts2movie --target-process SlayTheSpire2 --verify`: movie drift, unclassified screen, target mismatch report를 출력한다.
- `sts2-tas tas-verify --movie run.sts2movie --runs 5`: 같은 movie verification report를 N회 반복한다.
...
검색은 checkpoint 검증 실패 시 branch candidate에서 제외해야 한다.
