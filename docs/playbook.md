...
| Neow / event | After embark or event node | Dialogue `NAncientDialogueHitbox`, then `NEventOptionButton` ForceClick, then `NEventRoom.Proceed()` when finished. Never travel the map while the event room is still visible. | Yes: option index vs gold/HP |
