...
| Neow / event | After embark or event node | Ancient: ForceClick `%DialogueHitbox` until last line. Then ForceClick `Layout.OptionButtons[i]`. Only when `EventModel.IsFinished`, `await NEventRoom.Proceed()`. | Yes: option index vs gold/HP |
