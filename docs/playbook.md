...
| Neow / event | After embark or event node | Enabled unlocked `NEventOptionButton` via `OptionButtonClicked`/`Chosen` (skip dialogue hitbox — it throws). Prefer `IsProceed`, then room `ProceedButton` / `NEventRoom.Proceed()`. | Yes: option index vs gold/HP |
