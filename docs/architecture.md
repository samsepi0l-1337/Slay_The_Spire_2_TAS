...
v1 목표는 Windows interactive desktop에서 실제 Slay the Spire 2를 대상으로 같은 movie를 재생했을 때 drift 없이 같은 결과에 도달하는 TAS runtime을 만드는 것이다.
...
- Gate 5: `tas-verify --runs 5` reports five victories, zero drift, zero unclassified screen, zero target-window mismatch.
